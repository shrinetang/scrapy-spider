...
class DrugSpider(CrawlSpider):  
    ...
    rules = [ 
        ...
        Rule(LinkExtractor(allow=(".*((bw_t\d_v\d{1,2})|(bw_v\d{1,2}_p\d{1,2})).*")),follow=True,callback='parse_jb_menu'),
        #症状详细信息的抓取 URL类似于http://jbk.39.net/zhengzhuang/gjtt/
        Rule(LinkExtractor(allow=(".*/zhengzhuang/[a-zA-Z]+/.*")),follow=True,callback='parse_zhenzhuang'),
    ...
    def parse_zhenzhuang(self, response):
        err_num = 0
        knjb_dict = {}
        knjb_info_list = []
        ...
        if knjb_info_list: #这里有部分网页并非疾病信息页 过滤掉
            ...
            OutPut = ('zzjb' + '\t' + zzname + '\t' + zzprob + '\t' + zzansw +'\t' +str_knjb_info_list).encode('utf8').replace('\r\n','').replace('\n','').replace(' ','')
            print OutPut
    ...
    def parse_jb_menu(self, response):
        err_num = 0
        jb_name_list = []
